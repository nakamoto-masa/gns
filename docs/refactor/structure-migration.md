...
graph LR
    ...
    subgraph AFTER["<b>AFTER: Modular Structure</b>"]
        subgraph GNS["gns/ (Reusable Modules)"]
            NEW_CONFIG["config.py<br/>(250 lines)"]
            NEW_SIMULATOR["learned_simulator.py<br/>(400 lines)"]
            NEW_GRAPH["graph_model.py<br/>(250 lines)"]
            NEW_TRAINING["training.py<br/>(790 lines)"]
            NEW_ROLLOUT["rollout.py<br/>(305 lines)"]
            NEW_INFERENCE["inference_utils.py<br/>(210 lines)"]
            NEW_RENDER["render.py<br/>(350 lines)"]
        end

        subgraph SCRIPTS["scripts/ (CLI Wrappers)"]
            NEW_TRAIN_CLI["gns_train.py<br/>(180 lines)"]
            NEW_TRAIN_MULTI["gns_train_multinode.py<br/>(250 lines)"]
            NEW_RENDER_CLI["gns_render_rollout.py<br/>(56 lines)"]
        end
    ...
    class OLD_TRAIN,OLD_LEGACY oldStyle
    class NEW_CONFIG,NEW_SIMULATOR,NEW_GRAPH,NEW_TRAINING,NEW_ROLLOUT,NEW_INFERENCE,NEW_RENDER newModuleStyle
...
**主な変更:**
- `gns/train.py` (658行) + `gns/train_multinode.py` (661行) → 複数のモジュール + 薄いCLIラッパー
- `gns/render_rollout.py` (246行) → `gns/render.py` (350行) + `scripts/gns_render_rollout.py` (56行)
...
2. **Model Architecture** (既存のgns/learned_simulator.py → 分離)
   - LearnedSimulatorクラスの整理
   - GNNモデルコアをgns/graph_model.pyに分離
