"""Multi-GPU training script for GNS - thin CLI wrapper.

This script provides a command-line interface for distributed training
across multiple GPUs using DistributedDataParallel (DDP).
"""
...
from gns import config, reading_utils, training, rollout, distribute
...
def train_distributed(rank: int, flags_dict: dict, world_size: int, device: torch.device):
    """Train the model on multiple GPUs using DDP.

    Args:
        rank: Process rank for distributed training.
        flags_dict: Dictionary of configuration flags.
        world_size: Total number of processes.
        device: PyTorch device type (cuda).
    """
    ...
    # Get simulator wrapped with DDP
    simulator = training.get_simulator(
        metadata, flags_dict["noise_std"], flags_dict["noise_std"],
        simulator_config, device, rank=rank, is_distributed=True
    )
    ...
    train_dl = training.get_training_dataloader(
        ...
        flags_dict["batch_size"], is_distributed=True
    )
    ...
    training.run_training_loop(
        ...
        flags_dict["model_path"], is_distributed=True,
        ...
        if device == torch.device('cuda'):
            ...
            if FLAGS.n_gpus is None or FLAGS.n_gpus > available_gpus:
                ...
                if FLAGS.n_gpus is not None:
                    print(f"Warning: Requested {FLAGS.n_gpus} GPUs exceeds available {available_gpus} GPUs")
            else:
            ...
            distribute.spawn_train(train_distributed, flags_dict, world_size, device)

        # Train on CPU (not distributed)
        else:
            print("Training on CPU (not distributed)")
            from scripts.gns_train import train_single_gpu
